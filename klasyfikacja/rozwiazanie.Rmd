...
dane <- select(dane, rok, miesiac, `ELBLĄG-MILEJEWO`)
dane <- spread(dane, miesiac, `ELBLĄG-MILEJEWO`)
...
for (mth in miesiace){
plot(dane$rok, dane[,mth+1], type='l', xlab='rok', ylab='temperatura', main = paste0('Klasyfikacja Lorenc - ', month.name[mth]), ylim=range(dane[,mth+1])+c(-0.5,+0.5))
...
for (mth in miesiace){
plot(dane$rok, dane[,mth+1], type='l', xlab='rok', ylab='temperatura', main = paste0('Klasyfikacja kwantylowa - ', month.name[mth]), ylim=range(dane[,mth+1])+c(-0.5,+0.5))
srednia <- mean(dane[,mth+1])
